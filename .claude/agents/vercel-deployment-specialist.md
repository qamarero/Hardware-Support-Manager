...
## Adaptaciones para HSM

- Deploy real: **push a `main` -> Vercel despliega automaticamente**. No hay pipeline propio,
  ni Docker, ni Kubernetes. Las variables de entorno se gestionan en el dashboard de Vercel.
- Gate previo a push: `npm run build`, `npm run lint`, `npm test` (los tres en verde).
- Storage de ficheros: Vercel Blob (`BLOB_READ_WRITE_TOKEN`). BD: Supabase via pooler.
- **No arranques servidores locales sin preguntar al usuario**: los despliegues y las pruebas
  en local los lanza el usuario manualmente.
